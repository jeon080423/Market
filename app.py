...
try:
    ...
    def calculate_ml_lagged_weights(_ks_s, _sp_s, _fx_s, _b10_s, _cp_s, _ma20, _vx_s):
        ...
        for d in _ks_s.index[-252:]:
            s_sp = get_hist_score_val(_sp_s.shift(best_lags['SP']), d, True); s_fx = get_hist_score_val(_fx_s.shift(best_lags['FX']), d)
            s_b10 = get_hist_score_val(_b10_s.shift(best_lags['B10']), d); s_cp = get_hist_score_val(_cp_s.shift(best_lags['CP']), d, True)
    ...
    with c_gauge: 
        fig_gauge = go.Figure(go.Indicator(mode="gauge+number", value=total_risk_index, title={'text': "MARKET RISK INDEX", 'font': {'size': 20}},
        ...
        fig_gauge.update_layout(margin=dict(l=40, r=40, t=80, b=40), height=350); st.plotly_chart(fig_gauge, use_container_width=True)

    st.markdown("---")
    cn, cr = st.columns(2)
    with cn:
        st.subheader("📰 글로벌 경제 뉴스 통합 분석 (AI)")
        news_data = get_market_news()
        all_titles = ". ".join([a['title'] for a in news_data])
        if news_data:
            with st.spinner("AI가 뉴스를 정밀 분석 중입니다..."):
                # 프롬프트 강화: 문법 및 언어 제약 조건 추가
                news_prompt = f"""
                다음은 최근 경제 뉴스 제목들입니다: {all_titles}
                이 뉴스들을 종합하여 현재 시장의 주요 리스크와 투자자가 주의해야 할 핵심 포인트를 한국어로 분석해줘.
                
                [반드시 준수할 규칙]:
                1. 답변은 반드시 완결된 한국어 문장으로 작성할 것.
                2. '사건사', 'mercado'와 같은 엉터리 단어나 외래어를 섞지 말 것.
                3. 첫 문장은 현재의 전반적인 시장 분위기를 설명하고, 두 번째 문장은 구체적인 대응 전략을 제시할 것.
                4. 분석 결과에 'AI 뉴스 통합 분석'이라는 제목 문구를 반복해서 포함하지 말 것.
                """
                summary_text = get_ai_analysis(news_prompt)
                st.markdown(f'<div class="ai-analysis-box"><strong>🔎 시장 분석 리포트</strong><br><br>{summary_text.strip()}</div>', unsafe_allow_html=True)
            for a in news_data: st.markdown(f"- [{a['title']}]({a['link']})")

    with cr:
        st.subheader("💬 한 줄 의견(익명)")
        st.session_state.board_data = load_board_data()
        board_container = st.container(height=250) 
        with board_container:
            if not st.session_state.board_data: st.write("의견이 없습니다.")
            else:
                for post in st.session_state.board_data[::-1][:20]:
                    st.markdown(f"**{post.get('Author','익명')}**: {post.get('Content','')} <small>({post.get('date','')})</small>", unsafe_allow_html=True)

    # 9. 지표별 상세 분석 및 AI 설명
    st.markdown("---")
    st.subheader("🔍 실물 경제 및 주요 지표 종합 진단")
    latest_data_summary = f"- S&P 500: {sp_s.iloc[-1]:.2f}, 환율: {fx_s.iloc[-1]:.1f}원, 구리: {cp_s.iloc[-1]:.2f}, VIX: {vx_s.iloc[-1]:.2f}"
    
    with st.expander("🤖 현재 시장 지표 종합 진단", expanded=True):
        with st.spinner("데이터 기반 마켓 진단 중..."):
            diag_prompt = f"""
            다음 주식 시장 지표 데이터를 보고, 현재 한국 증시(KOSPI)에 미칠 영향과 투자 분위기를 쉽고 전문적인 한국어로 설명해줘.
            데이터: {latest_data_summary}
            
            [반드시 준수할 규칙]:
            1. 문법에 맞는 완벽한 한국어 문장만 사용할 것. 
            2. 지표별(S&P, 환율, 구리, VIX) 영향을 각각 별도의 문장으로 설명한 뒤 요약할 것.
            3. 각 문장은 가독성을 위해 줄바꿈을 적용할 것.
            4. 답변에 의미 없는 특수기호나 깨진 글자를 포함하지 말 것.
            """
            diag_text = get_ai_analysis(diag_prompt)
            # 가독성을 높이기 위해 텍스트 정제 및 레이아웃 적용
            st.markdown(f'<div class="ai-analysis-box" style="border-left-color: #ff4b4b;">{diag_text.replace(". ", ".<br><br>")}</div>', unsafe_allow_html=True)

    # 그래프 렌더링 섹션
    def create_chart(series, title, threshold, desc_text, color="blue"):
        fig = go.Figure(go.Scatter(x=series.index, y=series.values, name=title, connectgaps=True, line=dict(color=color)))
        fig.add_hline(y=threshold, line_width=2, line_color="red", annotation_text=desc_text)
        fig.update_layout(margin=dict(l=0, r=0, t=30, b=0), height=300)
        return fig

    r1_c1, r1_c2, r1_c3 = st.columns(3)
    r1_c1.plotly_chart(create_chart(sp_s, "S&P 500", sp_s.last('365D').mean()*0.9, "Risk Line", "royalblue"), use_container_width=True)
    r1_c2.plotly_chart(create_chart(fx_s, "KRW/USD", fx_s.last('365D').mean()*1.02, "Weak Line", "green"), use_container_width=True)
    r1_c3.plotly_chart(create_chart(cp_s, "Copper", cp_s.last('365D').mean()*0.9, "Demand Risk", "orange"), use_container_width=True)

    # KOSPI 기술적 분석 및 화살표 보정
    st.markdown("#### KOSPI 기술적 분석")
    ...
    fig_ks.add_annotation(x=ks_recent.index[-1], y=ma20.iloc[-1], text="20일 평균선 기준 지지/저항", showarrow=True, arrowhead=2, ay=-40, font=dict(color="red"), bgcolor="white", bordercolor="red")
