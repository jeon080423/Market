...
try:
    ...
    with c_gd:
        ...
        | :--- | :--- | :--- | :--- |
        | **0-40** | **Safe** | 매크로 및 심리 지표 안정. 외국인 유입 가능성 높음. | **적극적 수익 추구.** 주식 비중을 확대하고 주도주 위주의 공격적 포트폴리오 운용. |
        | **40-60** | **Watch** | 지표 혼조세. 추세 전환의 변곡점 가능성. 변동성 확대. | **현금 비중 조절 시작.** 추가 매수는 지양하고, 수익이 난 종목은 일부 차익 실현 고려. |
        | **60-80** | **Danger** | 주요 지표 악화(환율 급등, 금리 역전 등). 투매 심리 확산. | **방어적 운용 및 리스크 관리.** 주식 비중을 50% 이하로 축소. 변동성이 낮은 배당주나 가치주 위주 재편. |
        | **80-100** | **Panic** | 시장 붕괴 위험 고조. 극도의 공포(VIX 급등),기술적 지지선 붕괴. | **최우선 리스크 관리.** 가급적 현금 비중 최대화. 신용/미수 사용 전면 금지 및 손절매 기준 엄격 적용. |
    ...
    with cr:
        ...
        board_container = st.container(height=320) 
    ...
    def create_chart(series, title, threshold, desc_text):
        ...
        fig.add_vline(x=COVID_EVENT_DATE, line_width=1.5, line_dash="dash", line_color="blue"); return fig
